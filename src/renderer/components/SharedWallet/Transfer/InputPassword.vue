...
export default {
  ...
  mounted() {
    ...
    const that = this;
    dbService.wallet.findOne(
      { address: this.transfer.coPayers[0].address },
      function(err, doc) {
        if (err) {
          console.log(err);
        }
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.password && this.checked && !this.sending) {
        ...
        const pks = this.sharedWallet.coPayers.map(
          p => new Crypto.PublicKey(p.publickey)
        );
        //sponsorWallet
        ...
        try {
          pri = enc.decrypt(
            this.password,
            new Crypto.Address(this.sponsorWallet.address),
            this.sponsorWallet.salt,
            DEFAULT_SCRYPT
          );
        } catch (err) {
          this.sending = false;
